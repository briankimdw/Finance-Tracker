    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">{filtered.length} entries &middot; ${totalSpent.toFixed(2)} spent{totalPayments > 0 ? ` &middot; $${totalPayments.toFixed(2)} card payments` : ""}</p>
        </div>
        ...
        {topCategories.map(([cat, amount]) => (
          <div key={cat} className="bg-white border border-gray-200 rounded-xl p-4 shadow-sm">
            <div className="flex items-center gap-2 mb-1.5">
              ...
              <span className="text-xs text-gray-400 uppercase tracking-wider truncate">{cat}</span>
            </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 text-left">
                <th className="px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider">Expense</th>
                <th className="px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider">Category</th>
                <th className="px-4 py-3 text-xs font-medium text-gray-400 uppercase tracking-wider">Paid With</th>
                ...
                <tr>
                  <td colSpan={7} className="px-4 py-12 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-sm font-medium text-gray-400">{expenses.length === 0 ? "No expenses yet" : "No expenses match your filters"}</p>
                      <p className="text-xs text-gray-300 mt-1">Expenses you add will appear here</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filtered.map((exp) => {
                  ...
                  return (
                    <tr key={exp.id} className="hover:bg-gray-50/80 even:bg-gray-50/40">
                      ...
                      <td className="px-4 py-3.5 text-sm text-gray-500">{exp.category}</td>
                      <td className="px-4 py-3.5 text-sm">
                        {card ? (
                      ...
                      <td className={`px-4 py-3.5 text-sm font-medium ${exp.is_card_payment ? "text-green-600" : "text-red-600"}`}>
                        {exp.is_card_payment ? "+" : "-"}${Number(exp.amount).toFixed(2)}
                      </td>
                      <td className="px-4 py-3.5 text-sm text-gray-500">{new Date(exp.date).toLocaleDateString()}</td>
                      <td className="px-4 py-3.5 text-sm text-gray-500">{exp.recurring ? exp.frequency : "One-time"}</td>
                      <td className="px-4 py-3.5">
                        <button onClick={() => setEditExpense(exp)} className="text-gray-400 hover:text-blue-600 transition-colors mr-1" title="Edit"><Pencil size={14} /></button>
                        <button onClick={() => deleteExpense(exp.id)} className="text-gray-300 hover:text-red-500 transition-colors" title="Delete"><Trash2 size={14} /></button>
                      </td>
                    </tr>
                  );
                })
