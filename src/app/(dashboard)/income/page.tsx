...
export default function IncomePage() {
  ...
  const inputClass = "bg-white dark:bg-gray-900 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent";
    ...
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm mt-0.5 break-words">{filtered.length} entries · ${totalAll.toFixed(2)} total</p>
        </div>
        <button onClick={() => setShowAddModal(true)} className="bg-blue-600 hover:bg-blue-700 text-white font-medium py-2 px-4 rounded-lg flex items-center justify-center gap-2 transition-colors shrink-0 self-start sm:self-auto">
          <Plus size={18} /> <span>Add Income</span>
        </button>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-sm">
          ...
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">${totalAll.toFixed(2)}</p>
        </div>
        <div className="bg-white dark:bg-gray-900 border border-blue-100 dark:border-blue-800 rounded-xl p-4 shadow-sm">
          <span className="text-sm text-blue-600 dark:text-blue-400">Main Income</span>
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">${totalMain.toFixed(2)}</p>
        </div>
        <div className="bg-white dark:bg-gray-900 border border-purple-100 dark:border-purple-800 rounded-xl p-4 shadow-sm">
          <span className="text-sm text-purple-600 dark:text-purple-400">Side Income</span>
          <p className="text-2xl font-bold text-gray-900 dark:text-gray-100 mt-1">${totalSide.toFixed(2)}</p>
        </div>
      </div>
        ...
        {(["all", "main", "side"] as const).map((tab) => (
          <button key={tab} onClick={() => setActiveTab(tab)}
            className={`px-4 py-1.5 rounded-md text-sm font-medium transition-colors ${activeTab === tab
              ? tab === "main" ? "bg-blue-600 text-white shadow-sm" : tab === "side" ? "bg-purple-600 text-white shadow-sm" : "bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100 shadow-sm"
              : "text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100"}`}>{tab === "all" ? "All" : tab === "main" ? "Main" : "Side"}</button>
        ))}
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-xl p-4 shadow-sm">
        <div className="flex flex-wrap gap-3">
          ...
          {hasFilters && (
            <button onClick={clearFilters} className="flex items-center gap-1 bg-gray-100 dark:bg-gray-800 hover:bg-gray-200 dark:hover:bg-gray-700 border border-gray-300 dark:border-gray-700 rounded-lg px-3 py-2 text-sm text-gray-600 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-100 transition-colors"><X size={14} /> Clear</button>
          )}
        </div>
      </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200 dark:border-gray-800 text-left">
                <th className="px-4 py-3 text-xs font-medium text-gray-400 dark:text-gray-500 uppercase tracking-wider">Source</th>
                ...
                filtered.map((inc) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5">
                      <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${inc.type === "main" ? "bg-blue-50 dark:bg-blue-950/40 text-blue-600 dark:text-blue-400" : "bg-purple-50 dark:bg-purple-950/40 text-purple-600 dark:text-purple-400"}`}>{inc.type === "main" ? "Main" : "Side"}</span>
                    </td>
                    <td className="px-4 py-3.5 text-sm text-green-600 dark:text-green-400 font-medium">+${Number(inc.amount).toFixed(2)}</td>
                    <td className="px-4 py-3.5 text-sm text-gray-500 dark:text-gray-400">{new Date(inc.date).toLocaleDateString()}</td>
