    ...
    <div className="space-y-6">
      ...
      >
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">Track spending against monthly limits</p>
        </div>
        ...
        >
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <h2 className="text-sm font-semibold text-gray-900 uppercase tracking-wider">{monthName} Overall</h2>
            </div>
            <span className={`text-sm font-bold ${totalSpent > totalBudget ? "text-red-600" : "text-gray-900"}`}>
              {overallPct.toFixed(0)}%
            </span>
          </div>
          <div className="flex items-baseline gap-2 mb-3">
            <AnimatedNumber value={totalSpent} prefix="$" className={`text-3xl font-bold ${totalSpent > totalBudget ? "text-red-600" : "text-gray-900"}`} />
            <span className="text-sm text-gray-400 tabular-nums">of ${totalBudget.toFixed(2)}</span>
          </div>
          ...
          <div className="flex items-center justify-between mt-2 text-xs">
            ...
            <span className={totalSpent > totalBudget ? "text-red-600 font-medium" : "text-gray-500"}>
      ...
      {loading ? (
        ...
        >
          <div className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-sm font-semibold text-gray-700">No budgets set</p>
            <p className="text-xs text-gray-400 mt-1 max-w-xs text-center">Create category budgets to track your monthly spending against your goals</p>
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2">
                      <div>
                        <h3 className="text-base font-semibold text-gray-900">{b.category}</h3>
                        <p className="text-xs text-gray-400 mt-0.5 tabular-nums">
                          <AnimatedNumber value={b.spent} prefix="$" className="font-medium text-gray-700" />
                          <span> of ${Number(b.monthly_amount).toFixed(2)}</span>
                        </p>
                      </div>
                      <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                        <button onClick={() => { setEditBudget(b); setShowModal(true); }} className="text-gray-400 hover:text-blue-600 p-1.5 rounded" title="Edit"><Pencil size={14} /></button>
                        <button onClick={() => { if (confirm(`Delete ${b.category} budget?`)) deleteBudget(b.id); }} className="text-gray-300 hover:text-red-500 p-1.5 rounded" title="Delete"><Trash2 size={14} /></button>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between mt-2 text-xs">
                      ...
                      <span className={`tabular-nums ${b.overBudget ? "text-red-600 font-medium" : "text-gray-500"}`}>
