...
interface InviteTripMembersModalProps {
  isOpen: boolean;
...
export default function InviteTripMembersModal({ isOpen, trip, onClose, onInvite, onInviteFriend, onRemoveMember }: InviteTripMembersModalProps) {
  ...
  const buildJoinUrl = (token: string) => `${baseUrl}/trips/join/${token}`;
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    if (result.emailSent) {
      ...
      setNotice({ kind: "warn", text: `Email didn't send. ${friendly} Your invite link is ready in the list below — just hit Copy.` });
    }
    setEmail("");
  ...
  const handleRemoveMember = async (m: TripMember) => {
    if (!onRemoveMember) return;
    const ok = await confirm({
      title: "Remove this member from the trip?",
      destructive: true,
      confirmLabel: "Remove",
    });
    if (!ok) return;
    try {
      await onRemoveMember(trip.id, m.user_id);
      info("Member removed");
    } catch {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white dark:bg-gray-900 rounded-2xl w-full max-w-md max-h-[90vh] overflow-y-auto shadow-2xl shadow-gray-900/10 border border-gray-100 dark:border-gray-800">
        <div className="flex items-center justify-between px-5 py-4 border-b border-gray-100 dark:border-gray-800 sticky top-0 bg-white dark:bg-gray-900 z-10">
          <div>
            <h2 className="text-lg font-semibold text-gray-900 dark:text-gray-100">Travel companions</h2>
            <p className="text-xs text-gray-400 dark:text-gray-500 truncate">{trip.name}</p>
          </div>
        ...
        <div className="p-5 border-b border-gray-100 dark:border-gray-800">
          <div className="flex items-center justify-between mb-3">
            <h3 className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider flex items-center gap-1.5"><Users size={12} /> Invite a friend</h3>
            <Link href="/friends" className="text-[11px] text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 font-medium">Manage →</Link>
          </div>

          {friends.length === 0 ? (
            <div className="bg-blue-50 dark:bg-blue-950/40 border border-blue-100 dark:border-blue-800 rounded-lg p-3">
              <div className="flex items-start gap-2">
                <Sparkles size={14} className="text-blue-600 dark:text-blue-400 mt-0.5 shrink-0" />
                <div>
                  <p className="text-xs font-semibold text-blue-900 dark:text-blue-200">No friends yet</p>
            ...
            <>
              <div className="relative mb-2">
                ...
                <input type="text" value={friendQuery} onChange={(e) => setFriendQuery(e.target.value)}
                  ...
                  className="w-full bg-white dark:bg-gray-900 border border-gray-200 dark:border-gray-800 rounded-lg pl-7 pr-3 py-2 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 dark:focus:border-blue-500 text-xs" />
              </div>
              {invitableFriends.length === 0 ? (
                ...
                <div className="space-y-1.5 max-h-56 overflow-y-auto">
                  {invitableFriends.map((f) => {
                    ...
                    return (
                      <div key={f.userId} className="flex items-center gap-2 p-2 rounded-lg bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                        <div className="w-8 h-8 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center shrink-0 text-xs font-semibold text-blue-700 dark:text-blue-300">{initial}</div>
                        <div className="flex-1 min-w-0">
                          <p className="text-sm font-medium text-gray-900 dark:text-gray-100 truncate">{f.profile?.display_name || f.profile?.username || "Friend"}</p>
                          <p className="text-[11px] text-gray-400 dark:text-gray-500 truncate">{f.profile?.username ? <>@{f.profile.username}</> : f.profile?.email}</p>
                        </div>
                        {isPending ? (
                          <span className="text-[10px] font-semibold text-amber-600 dark:text-amber-400 flex items-center gap-1 px-2 py-1"><Clock size={10} /> PENDING</span>
                        ) : (
              ...
              <p className="text-[11px] text-gray-400 dark:text-gray-500 mt-2">They&apos;ll see an invite on their Friends page and have to accept it to join.</p>
            </>
          )}
        </div>

        {/* Email invite */}
        <form onSubmit={handleInvite} className="p-5 space-y-3 border-b border-gray-100 dark:border-gray-800">
          <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">
            <span className="flex items-center gap-1.5"><Mail size={14} className="text-gray-400 dark:text-gray-500" /> Or invite by email</span>
          </label>
          ...
          {notice && (
            <div className={`flex items-start gap-2 text-xs rounded-lg px-3 py-2 ${
              ...
              notice.kind === "warn" ? "bg-amber-50 dark:bg-amber-950/40 text-amber-700 dark:text-amber-300 border border-amber-100 dark:border-amber-800" :
          ...
          <p className="text-[11px] text-gray-400 dark:text-gray-500">Use email for people who aren&apos;t your friends yet or don&apos;t have a NetWorth account. Reply-To is your email ({user?.email}).</p>
        </form>

        {pending.length > 0 && (
          <div className="p-5 border-b border-gray-100 dark:border-gray-800">
            <h3 className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3">Pending email invites</h3>
            <div className="space-y-2">
              {pending.map((inv) => (
                <div key={inv.id} className="flex items-center gap-2 p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800">
                  <div className="w-7 h-7 rounded-full bg-amber-100 dark:bg-amber-900/50 flex items-center justify-center shrink-0"><Mail size={12} className="text-amber-600 dark:text-amber-400" /></div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-900 dark:text-gray-100 truncate">{inv.email || "(no email)"}</p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500">Expires {new Date(inv.expires_at).toLocaleDateString()}</p>
                  </div>
                  <button onClick={() => handleCopy(inv.token)} className="text-xs text-blue-600 dark:text-blue-400 hover:text-blue-700 dark:hover:text-blue-300 p-1.5 rounded" title="Copy link">
                    {copied === inv.token ? <Check size={13} /> : <Copy size={13} />}
                  </button>
                  <button onClick={() => handleRevoke(inv.id)} className="text-gray-300 dark:text-gray-600 hover:text-red-500 p-1.5 rounded" title="Revoke"><Trash2 size={13} /></button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="p-5">
          <h3 className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wider mb-3 flex items-center gap-1.5">
            <Users size={12} /> Members ({trip.members.length})
          </h3>
          <div className="space-y-2">
            {trip.members.map((m) => {
              ...
              return (
                <div key={m.id} className="flex items-center gap-2 p-2.5 rounded-lg bg-gray-50 dark:bg-gray-800">
                  <div className="w-7 h-7 rounded-full bg-blue-100 dark:bg-blue-900/50 flex items-center justify-center shrink-0 text-[11px] font-semibold text-blue-700 dark:text-blue-300">{initial}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm text-gray-900 dark:text-gray-100 truncate">
                      {isMe ? "You" : displayName}
                      {m.role === "owner" && <span className="ml-1.5 text-[10px] font-semibold text-amber-600 dark:text-amber-400">OWNER</span>}
                    </p>
                    <p className="text-[10px] text-gray-400 dark:text-gray-500">{p?.username ? <>@{p.username} · </> : ""}Joined {new Date(m.joined_at).toLocaleDateString()}</p>
                  </div>
                  {trip.isOwner && m.role !== "owner" && onRemoveMember && (
                    <button onClick={() => handleRemoveMember(m)} className="text-gray-300 dark:text-gray-600 hover:text-red-500 p-1.5 rounded" title="Remove"><Trash2 size={13} /></button>
                  )}
                </div>
              );
            })}
